...
const displayData = (apply) => {
    ...
    for (let i = 0; i < apply.length; i++) {
        ...
        // employeeTypeCell.textContent = apply.user;

        // var vacancyEndDateCell = document.createElement('td');
        // vacancyEndDateCell.textContent = Apply.vacancyEndDate.split('T')[0];
        ...
        for (const key in project) {
            if (project.hasOwnProperty(key)) {
                ...
                if (key === 'time_duration' && typeof value === 'object') {
                    ...
                    propertyElement.innerHTML = `<strong>${key}:</strong> ${Array.isArray(value) ? value.join(', ') : value}`;
